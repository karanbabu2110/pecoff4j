...
public class DataReader implements IDataReader {
  ...
  public DataReader(final byte[] buffer) {
    this.dis = new BufferedInputStream(new ByteArrayInputStream(buffer));
  }

  public DataReader( final byte[] buffer, final int offset, final int length) {
  ...
  public DataReader(InputStream is) {
    ...
    try {
      ...
      throw new IllegalStateException("Error setting mark on stream", e);//NO i18N
    }
  }
  ...
  public int getLength() {
    try {
      ...
      throw new IllegalStateException("Error getting stream length", e);//NO i18N
    }
  }
  ...
  public String readUtf() throws IOException {
    ...
    while ((c = readByte()) != 0) {
      if (c == -1) {
        throw new IOException("Unexpected end of stream");//NO i18N
      }
      sb.append((char) c);
    }
    return sb.toString();
